...
class P17Test extends FunSpec with Matchers {
  ...
  describe("P17 Spec") {
    ...
    it("should split into two halves by length") {
      val (first, second) =
        P17.split(List("a", "b", "c", "d", "e", "f", "g", "h", "i", "k"), 3)
    ...
    it(
      "should split into two halves by length with size 0 and list size when n is 0") {
      val (first, second) =
        P17.split(List("a", "b", "c", "d", "e", "f", "g", "h", "i", "k"), 0)
  ...
  describe("P17 Spec take-drop") {
    ...
    it("should split into two halves by length") {
      val (first, second) = P17.split_take_drop(
